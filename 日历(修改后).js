...
function createTable(){
    ...
    $(document).ready(function(){
        $('#dateTable tr').each(function(i){
            $(this).children('td').each(function(j){ 
            var dateStr2 = (new Date(year,month,(i*7+j-firstDay+1))).toISOString();
            var tdMemo2 = localStorage.getItem(dateStr2);
            if(tdMemo2!=null) $(this).css("text-decoration","underline");})
        })
    });
    if(rows==5){
        var tr = document.createElement("tr");
...
function clean(){
    ...
    clearDate<=monthDays[clearMonth-1] && clearYear!="" && clearMonth!="" && clearDate!="" && clearMemo!=null){
